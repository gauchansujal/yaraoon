...
# Execute this SQL code to create a 'reservations' table
cursor.execute('''
...
def change():
    ...
    # Check if the selected seat is not blank
    if not selected_option:
        messagebox.showwarning("Warning", "Please select a valid seat.")
        return

    # Check if the seat is already reserved
    cursor.execute("SELECT * FROM reservations WHERE seat = %s", (selected_option,))
    if cursor.fetchone():
        messagebox.showwarning("Warning", f"The seat {selected_option} is already reserved.")
        return

    # Insert the selected seat into the database
    cursor.execute("INSERT INTO reservations (seat) VALUES (%s)", (selected_option,))
    conn.commit()

    result = messagebox.askyesno("Change", f"Are you sure you want to buy {selected_option}?")
    if result:
...
# Modify the button creation for each seat to check its reservation status
def create_seat_button(frame, seat_text):
    cursor.execute("SELECT * FROM reservations WHERE seat = %s", (seat_text,))
    is_reserved = cursor.fetchone()

    if is_reserved:
        # Seat is reserved, highlight in red
        seat_button = Button(frame, text=seat_text, bg="red", cursor="hand2", state=DISABLED)
    else:
        # Seat is not reserved, normal appearance
        seat_button = Button(frame, text=seat_text, bg="#D3D3D3", cursor="hand2", command=lambda: choose(seat_text))

    seat_button.grid(row=row, column=column)
    return seat_button

# Create seat buttons for the first row
row = 1
column = 0
seat_text1="A7"
...
def on_seat_select(event):
    selected_seat = combo.get()
    save_selected_seat(selected_seat)

def save_selected_seat(selected_seat):
    with open("sel_seat.txt", "w") as file:
        file.write(selected_seat)
...
combo.pack()
combo.bind("<<ComboboxSelected>>", on_seat_select)
